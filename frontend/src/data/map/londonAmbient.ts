...
interface Zone { lat: number; lng: number; weight: number; radius: number; label: string }
...
function pickTag(rng: () => number): string {
  let r = rng();
  for (const t of TAGS) { r -= t.weight; if (r <= 0) return t.tag; }
  return 'Common';
}
...
function generateCorridors(count: number, rng: () => number): WesternFragment[] {
  const corridors: [Zone, Zone][] = [
    [ZONES[0], ZONES[1]], [ZONES[0], ZONES[3]], [ZONES[0], ZONES[7]],
    [ZONES[4], ZONES[0]], [ZONES[0], ZONES[2]],
  ];
  const out: WesternFragment[] = [];
  ...
  for (const [a, b] of corridors) {
    for (let i = 0; i < per && out.length < count; i++) {
      ...
      out.push({
        ...
        subtitle: 'London, ON',
        tag: pickTag(rng),
        lat: a.lat + (b.lat - a.lat) * t + (rng() - 0.5) * 0.003,
        lng: a.lng + (b.lng - a.lng) * t + (rng() - 0.5) * 0.003,
      });
    }
  }
  return out;
...
const _ambient = generateAmbient(180);
const _corridors = generateCorridors(40, mulberry32(SEED + 13));
